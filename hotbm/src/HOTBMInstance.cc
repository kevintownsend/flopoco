...
{
  tParam paramKey(f.getShortName(), p);
  for (int i = 0; i <= p.n; i++) {
  ...
  if (approxCache.count(paramKey)) {
    ...
    {
      ostringstream buf;
      buf << "cache/" << f.getShortName() << "_" << p.wI << "_" << p.alpha << "_" << p.n << ".minimax";
    ...
    for (long long int i = 0; i < P(p.alpha); i++) {
      ...
      Polynomial pTaylorDiff;
      for (int j = 0; j <= 16; j++)
	pTaylorDiff = pTaylorDiff + Polynomial(j, f.eval((ia+ib_)/2, j) / fact(j));
      for (int j = 0; j <= p.n; j++)
...
void HOTBMInstance::buildTerms()
{
  ...
  for (int i = 0; i <= p.n; i++) {
    tTermParam paramKey(tParam(f.getShortName(), p), i);
    for (int j = 0; j <= p.n; j++) {
      if (j == i)
	continue;
...
void HOTBMInstance::genVHDL(ostream &os, string name)
{
  ...
  os << "-- Command line: " << f.getShortName() << " ";
  p.print(os);
  os << endl;
  os << endl << endl;
