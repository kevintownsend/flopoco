...
How does it work? 
\begin{itemize}\item 
  \operator\ has a \verb!currentCycle! attribute, initially equal to
  zero. The main function of 	\verb!nextCycle()! is to increment \verb!cycle!.
...
\item Wrapping a signal in \verb!use()! has the following  effect. First,
  ...
  \verb!cycle! declared for \verb!X!. 
  \begin{itemize}\item 
    If they are equal, or if \verb!-pipeline=no!, \verb!use("X")! will
    simply return \verb!"X"!.
  \item If \verb!currentCycle! $<$ \verb!cycle!, \verb!use("X")!
  ...
  \item If \verb!currentCycle! $>$ \verb!cycle!, \verb!use("X")!
    ...
    n=\verb!currentCycle!$-$\verb!cycle! cycles. Technically
    \verb!use("X")! just returns \verb!"X_dn"!. In this case,
    it also updates the \verb!lifeSpan! of \verb!X! to be at least equal to n.
  \end{itemize}
\end{itemize}
...
inserting a single \verb!nextCycle()! in the code.

It is also possible to have \verb!if!s
before some of the \verb!nextCycle()!, so that the pipeline adapts to
the frequency, the operator generic parameters, etc.
...
\item Other functions allow to manipulate \verb!currentCycle!. They
  ...
  and \verb!syncCycleFromSignal(string s)! which allows to synchronise
  ...
  such synchronisations. All these functions have an optional boolean
...
(...)
   -- vhdl << declare("T", 64) << " <= Y * Z;" << endl;
	...
	inPortMap   (my_mult, "X", "Y");
	inPortMap   (my_mult, "Y", "Z");
	outPortMap  (my_mult, "R","T");
	vhdl << instance(my_mult, "my_mult"); // the VHDL instance name
   ...
   -- vhdl << "R <= " << use("X") << " + " << use("T") << ";" << endl;
	...
	oplist.push_back(my_adder); // some day this will be an addOperator method
	inPortMap   (my_adder, "X", "X");
	inPortMap   (my_adder, "Y", "T");
	inPortMapCst(my_adder, "Cin", "0"); -- carry in
	outPortMap  (my_adder, "R","RR");
	vhdl << instance(my_adder, "my_add"); // the VHDL instance name
   // advance to the cycle of the result
