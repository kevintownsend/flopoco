...
namespace flopoco
{
	...
	void BitHeap::compress(int stage)
	{
		...
		{
			cout << "stage " << stage << " iteration number " << lapCount++ << endl;
			...
			while(index<maxWeight){
				...
				cout << tab << "index = " << index << " maxWeight = " << maxWeight << endl;
				...
				cout << "computed adder length" << endl;

				if((endAddChain > index) && (endAddChain-index >= minAdd3Length))
				{

					cout << tab << tab << "found adder from index = " << index << " endAddChain = " << endAddChain << endl;
					cout << tab << tab << "found adder of size " << endAddChain-index << endl;
					...
					cout << "inputs to the addition are formed" << endl;
