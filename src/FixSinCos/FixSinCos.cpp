...
FixSinCos::FixSinCos(Target * target, int w_, float ratio):Operator(target), w(w_){
	...
	REPORT(0*DEBUG, "Boundaries on the various cases for w="<<w << ": " << wSmallerThanBorder1 << wSmallerThanBorder2 << wSmallerThanBorder3 << wSmallerThanBorder4);

	if (wSmallerThanBorder1 || (!wSmallerThanBorder2 && wSmallerThanBorder3))
	{
		REPORT(DETAILED, "Simpler architecture: Using plain table LutInputs=" << target->lutInputs() << "; sizeOfMemoryBlock=" << target->sizeOfMemoryBlock() << ";" );
		SinCosTable* scT= new SinCosTable(target, w+1, 2*(w+1), 1, this);
		...
		//		REPORT(0, "########## WARNING! This ressource was not freed: SinCosTable scT, adress=" << scT << " ##########");
		...
		/*********************************** REDUCED TABLE **************************************/
		SinCosTable* scT= new SinCosTable(target, w-1, 2*(w+1), 4, this);
	...
	/*		else if ((w+1)*2*2^(w+1)<target->sizeOfMemoryBlock() +1 )
	//TODO: implement same code than the first lutInputs lookup
	{}
	else if ((w+1)*2*2^(w+1-3)<target->lutInputs() +1 )
	//TODO: implement the same code than the second lutInputs lookup
	{

	}*/
	else if (1) //TODO: define input case
	{
		...
		vhdl << declare("C_sgn") << " <= X_sgn xor Q;" << endl;
		...
		const int wA=9; //arbitrary set for the moment. TODO: produce a clever definition of wA
		...
		/*********************************** REDUCED TABLE **************************************/
		SinCosTable* scT= new SinCosTable(target, wA, 2*(w+1+g), 8, this);
		...
		vhdl << tab << declare("SA", w+1+g) << " <= SCA " << range( 2*(w+1+g)-1 , w+1+g ) << ";" << endl;// signal declaration
		vhdl << tab << declare("CA", w+1+g) << " <= SCA " << range( w+g, 0 ) << ";" << endl;// signal declaration
		//delete (scT);
...
//		vhdl << declare("Z_op", Y_red_sqr_size) << "<= '0' & "<< og(Y_red_sqr_size-1)<< " - ( '0' & Y_red_sqr" << range(Y_red_sqr_size-1,1) << " );" << endl;
		int m=4;
		//---------------------------- Sine computation ------------------------
		vhdl << declare("SinACosY_red",w+1+g) << " <= SA;"<<endl;
		vhdl << declare("CosASinY_red", 2*wZ+m ) << " <= CA" << range( w+g, w+1+g-wZ-m ) <<"*Z;" <<endl;
		vhdl << declare("PreSinX", w+1+g) << " <= SinACosY_red + ( " << zg(wA-2+1) << " & CosASinY_red" << range( 2*wZ+m-1, wZ+m-1 ) << " );"<<endl;

		//---------------------------- Cosine computation -------------------------------
		vhdl << declare("CosACosY_red", w+1+g ) << " <= CA;" << endl;
		vhdl << declare("SinASinY_red", 2*wZ+m ) << " <= SA" << range( w+g, w+1+g-wZ-m ) << "*Z;" << endl;
		vhdl << declare("PreCosX", w+1+g) << " <= CosACosY_red - ( " << zg(wA-2+1) << " & SinASinY_red" << range( 2*wZ+m-1, wZ+m-1 )<< " );" << endl;

		//--------------------------- Reconstruction of both sine and cosine -----------------------------

		vhdl << tab << declare ("S_wo_sgn", w+1+g)
			<< " <= PreCosX when Exch = '1' else PreSinX;" << endl; //swap sine and cosine if q xor o.
		vhdl << tab << declare ("C_wo_sgn",w+1+g)
			<< " <= PreSinX when Exch = '1' else PreCosX;" << endl;


		vhdl << tab << declare ("S_wo_sgn_ext", w+1)
			<< " <= S_wo_sgn" << range(w+g,g) << ";" << endl
			<< tab << declare ("C_wo_sgn_ext", w+1)
			<< " <= C_wo_sgn"<<range(w+g, g)<<";" << endl; //S_wo_sgn_ext and C_wo_sgn are the positive versions of the sine and cosine
	...
	else if (!Check_Size)
	{
		//TODO:
		REPORT(0,"#################################################################################");
		REPORT(0,"GENERATING A TABLE WITH NEW ARGUMENT REDUCTION:");
		REPORT(0,"	ONLY FIRST ORDER OF TAYLOR TOKEN INTO ACCOUNT, PLUS SECOND ORDER FOR COSINE");
		REPORT(0,"#################################################################################");
		/*********************************** RANGE REDUCTION **************************************/ 
		// the argument is reduced into (0,1/4) because one sin/cos
		// computation in this range can always compute the right sin/cos
		...
		/*********************************** REDUCED TABLE **************************************/
		SinCosTable* scT= new SinCosTable(target, wA, w+ g, 8, this);
...
#define TINKERCOS 1
		...
		//     << " <= X_sgn;" << endl;
		vhdl << tab << declare("C_sgn")
			<< " <= Q xor X_sgn;" << endl; //sign of cosine
