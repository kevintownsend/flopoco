...
IntMultiplier:: IntMultiplier(Target* target, int wInX, int wInY) :
	...
	if(is_sequential()) 
	{
		...
		int multiplier_width_X, multiplier_width_Y;
		bool test = target->suggest_submult_size(multiplier_width_X, multiplier_width_Y, wInX, wInY);
		
		cout<<endl<<" test="<<test<<" suggested sizes X="<<multiplier_width_X<<" suggested sizes Y="<<multiplier_width_Y<<endl;
		
		 
		multiplier_width = multiplier_width_X;
		...
		for (i=1;i<=partsX;i++){
			name.str("");
			name <<"X_"<<i;
			add_registered_signal_with_sync_reset(name.str(), multiplier_width); 
		}
		...
		else
		{
			...
			//Instantiate an IntMultiplier -> multiply the significands
				...
				intadd = new IntAdder(target, wInX);
				...
				add_signal("addition_term", partsX  * multiplier_width);

				add_signal("addition_result", partsX  * multiplier_width);
			}
	...
	if (is_sequential()) { 
		...
		if (partsY==1) {
			...
			if (partsX>1){
				...
				o<<"addition_term <= ("<< zero_generator(multiplier_width,0)<<" & Low_1("<< partsX*multiplier_width-1<<" downto "<< multiplier_width<<"));"<<endl;
				...
				o<<tab<< "             rst => rst " << endl;
				o<<tab<< "               );" << endl<<endl;
				o<<tab<<"temp_result <= addition_result & Low_1("<<multiplier_width-1<<" downto 0 )"<<";"<<endl;
			}
			else
				o<<tab<<"temp_result <= High_1 & Low_1;"<<endl;
