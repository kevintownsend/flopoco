...
IntMultiplier:: IntMultiplier(Target* target, int wInX, int wInY) :
  ...
  if(is_sequential()) 
  {
    multiplier_width = target->mult_x_inputs();
    ...
    for (i=1;i<=partsY;i++)
      for (j=1;j<=partsX;j++){
    		name.str("");;
    		name <<"Y_"<<i<<"_X_"<<j;
    		add_registered_signal_with_reset(name.str(), 2 * multiplier_width);	
    	}	 
    ...
    //declare the signals which form the two trees (high and low)
      ...
      for (i=1;i<=partsY;i++)	{
		    name.str("");;
		    name<<"Buffer_H_"<<i;
		    add_registered_signal_with_reset(name.str(), partsX * multiplier_width);	
      }
      ...
      for (j=0; j<partsY;j++)	
        for (i=1;i<=partsY-j;i++)	{	
          ...
          add_registered_signal_with_reset(nameL.str(), partsX * multiplier_width);	
          add_registered_signal_with_reset(nameH.str(), partsX * multiplier_width);	
        }	
	
      //one more registers for the Low part for synchronization 
	    add_registered_signal_with_reset("Low1", partsX * multiplier_width);	
	 		
      //one more signal for the high part
	    add_signal("High1", partsX * target->mult_x_inputs());	
      ...
      //pipeline levels
    	for (j=1; j<=partsY;j++)	
    		for (i=1;i<=j;i++){	
			    ...
			    add_registered_signal_with_reset(name.str(), multiplier_width + 1 );	
		    }
    	...
    	add_signal("temp_result", (partsX + 1) * target->mult_x_inputs() );
...
void IntMultiplier::output_vhdl(std::ostream& o, std::string name) {
  ...
  if (is_sequential()) { 
    ...
    else
    {
      ...
      o<<endl<<endl;
      //compute the partial result as Low1 + High1 + carryIn bit from PartialBits_Level_(patrsY)_Reg_(partsY) 	
      o<<tab<<"temp_result <= Low1_d + High1 + PartialBits_Level_"<< partsY <<"_Reg_"<< partsY<<"_d("<< multiplier_width <<");"<<endl;
      ...
      o<<tab<<"partial_bits <= "<<the_bits.str()<<";"<<endl;
