...
//		int version = 1; /* this will go into the parameters */
		...
		if (isSequential()){
			...
			if (2*chunkSize_ >= wIn_){
				cerr << "ERROR FOR NOW -- instantiate int adder, dimmension too small for LongIntAdderCmpAddAdd" << endl;
				exit(0);
			}
			...
			for (int j=0; j<nbOfChunks; j++){
				if (j>0){ //for all chunks greater than zero we perform this comparissons
					vhdl<<tab<<declare(join("sX",j,"_0_l",l,"_Zero"), cSize[j]+1)<< " <= (\"0\" & " << join("sX",j,"_0_l",l-1)
			...
			for (int i=nbOfChunks-3; i>=0; i--) {
				vhdl << "sX"<<i+1<<"_0_l"<<l<<"_Zero"<<of(cSize[i])<< (i>0?" & ":";") ;
			} vhdl << endl;
