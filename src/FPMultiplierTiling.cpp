...
namespace flopoco{
	...
	Operator(target), wEX_(wEX), wFX_(wFX), wEY_(wEY), wFY_(wFY), wER_(wER), wFR_(wFR) {
		...
		nextCycle();/////////////////////////////// -> this one we may need as IntTilingMult is not flopoco 2.0 compatible	
		setCriticalPath(0.0);
		if (normalized_==true){
			/******************************************************************/
			
			vhdl << tab<< declare("norm") << " <= sigProd" << of(wFX_+wFY_+1) << ";"<<endl;
			double cpnorm = getCriticalPath();
			manageCriticalPath(target->localWireDelay() + target->adderDelay(wEX_+2));
			...
			if (1+wFR_ >= wFX_+wFY_+2) {  /* => no rounding needed - possible padding */
				...
				double cpfinalExc =getCriticalPath();
				
				syncCycleFromSignal("norm");
				setCriticalPath(cpnorm);
				manageCriticalPath(target->localWireDelay() + target->lutDelay()); 
				...
				if (syncCycleFromSignal("finalExc")){
					if (getCycleFromSignal("finalExc") == getCycleFromSignal("resSig"))
						setCriticalPath(max( getCriticalPath(), cpfinalExc));
				}
			...
			//TODO IF anyone needs it sometime
