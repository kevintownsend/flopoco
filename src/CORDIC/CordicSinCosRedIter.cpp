...
namespace flopoco{
		...
		: Operator(target), w(w_)
	{
		...
		vhdl << tab << declare("paddedShortCX", wcs) << " <= (" << wcs-1 << " downto " << wx << " => CX(" << wx-1 << ")) & shortCX;"  << endl;
		vhdl << tab << declare("paddedShortSX", wcs) << " <= (" << wcs-1 << " downto " << wx << " => SX(" << wx-1 << ")) & shortSX;"  << endl;
		
		vhdl << tab << declare("CsubShortSX", wcs) << " <= " << join("C", stage) << " - paddedShortCX;" << endl;
		vhdl << tab << declare("SaddShortCX", wcs) << " <= " << join("S", stage) << " + paddedShortSX;" << endl;
		...
		vhdl << tab << declare("preRoundedIntCout", 1+w+1) << "<= " << "CsubShortSX" << "(" << wcs-1 << " downto " << guardcs-1 << ");" << endl;
		vhdl << tab << declare("preRoundedIntSout", 1+w+1) << "<= " << "SaddShortCX" << "(" << wcs-1 << " downto " << guardcs-1 << ");" << endl;
	...
	std::string CordicSinCosRedIter::generateFixPointNumber(float x, int wI, int wF)
	{
		std::string result;
		int size = wI+wF;
		float xcopy = x;
	...
	std::string CordicSinCosRedIter::generateFixPointNumber(mpf_t x, int wI, int wF)
	{
		std::string result;
		int size = wI+wF;
		mpfr_t mx;
