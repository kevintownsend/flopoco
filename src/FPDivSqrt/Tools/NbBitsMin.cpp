...
int main (int argc, char** argv)
{
	if(argc == 3)
	{
		float radix = atof(argv[1]);
		cout<<endl<<"You're working in radix"<<radix<<endl;
		float digitSet = atof(argv[2]);
		cout<<"Your digit set is [-"<<digitSet<<","<<digitSet<<"]"<<endl;
	...
	{
		cout<<"This exe computes the minimum number of bits required as LUT's input(without prescaling) for an implementation of additive division"<<endl;
		cout<<"It needs two args : "<<endl<<"1) The radix"<<endl<<"2) The digit set range"<<endl;
		cout<<"Try something like :"<<endl<<"./nbBit 4 3"<<endl;
		return 0;
	}
	else
	{
		cout<<"Wrong number of arguments. Try ./nbBit help for further explanations."<<endl;
	}
}
...
bool checkDistrib(int delta, int t, int radix, int digitSet)
{
	...
	bool ok = true;
	const int hDiv = pow(2,t);
	const int vDiv = pow(2,delta-1);
